	...
	try {
		...
		$.ajax({
			...
			success : after_upload_success = function(req) {
					var file = (new Function ('('+req+')'))();  //eval('('+req+')');
					...
					(new Function ("preview(file.bf_file)"))();  // eval("preview(file.bf_file)");
				}
			});
	    ...
	    for (var i=0; i<obj.options.length; i++) {
			if (obj.options[i].selected == true) {
				...
				$.ajax({
					...
					url: url,
					data: params,
					success : (new Function ("delete_file_complete"))(),  //eval("delete_file_complete")
					});
...
function delete_file_complete() {
	try {
		...
		(new Function ("preview()"))();// eval("preview()");
	} catch (ex) {
		this.debug(ex);
	};
}
