...
class KCAPTCHA{
	...
	function image(){
        ...
        for($i=0;$i<$length;$i++){
            ...
            $y=(($i%1)*$fluctuation_amplitude - $fluctuation_amplitude/1)*$odd
		...
		for($i=0;$i<(($height-30)*$x)*$white_noise_density;$i++){
			imagesetpixel($img, mt_rand(0, $x-1), mt_rand(10, $height-10), $white);
		}
		for($i=0;$i<(($height-30)*$x)*$black_noise_density;$i++){
			imagesetpixel($img, mt_rand(0, $x-1), mt_rand(10, $height-10), $black);
		}
		...
		imagefilledrectangle($img2, 0, $height+10, $width-1, $height+12, $foreground);
		...
		// amplitudes
		$rand9=mt_rand(330,420)/210;
		$rand10=mt_rand(330,450)/210;
...
function captcha_html($class="captcha")
{
    ...
    $html .= '<input type="text" name="captcha_key" id="captcha_key" class="form-control" size="4" maxlength="4" placeholder="'.$_LANG[PL_WRITE_KEY].'">';//请输入验证码
    //$html .= "\n".'<img src="#" alt="" id="captcha_img">';
    $html .= "\n".'<span class="input-group-btn"><img src="javascript:void(0);" alt="" id="captcha_img" class="">';
    $html .= "\n".'<button type="button" id="captcha_reload" class="btn btn-dark"><i class="fa fa-refresh"></i> '.$_LANG[TX_REFRESH].'</button></span>';//刷新
    //if (!G5_IS_MOBILE) $html .= "\n".'<span class="input-group-btn"><button type="button" id="captcha_mp3" class="btn btn-danger"><i class="fa fa-volume-down"></i> </button></span>';
    $html .= "\n".'</div>';
    $html .= "\n".'</fieldset>';
    return $html;
}
