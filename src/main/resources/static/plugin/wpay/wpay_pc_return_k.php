...
try {
    ...
    if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
        ...
        try {
            ...
            extract($_REQUEST, EXTR_PREFIX_ALL, "re");

            // 결제 성공시 저장
            $qry   = " insert into pay_tran_log ( ";
            $qry  .= "   tid, tr_stat, result_code, result_msg, event_code, tot_price, moid, pay_method, appl_num, appl_date, appl_time, buyer_email, buyer_tel, buyer_name, card_num, currency";
            $qry  .= " ) values ( ";
            $qry  .= " '{$re_tid}', 'S', '{$re_resultCode}', '{$re_resultMsg}', '{$re_EventCode}', '{$re_TotPrice}', '{$re_MOID}', '{$re_payMethod}', '{$re_applNum}', '{$re_applDate}', '{$re_buyerEmail}', '{$re_buyerTel}', '{$re_buyerName}', '{$re_CARD_Num}', '{$re_currency}' ";
            $qry  .= " ) ";
    ...
    <body class="wrap">
        ...
        <main class="col-8 cont" id="bill-01">
            ...
            <section class="menu_cont mb-5">
				...
				<button onclick="location.href='wpay_pc_req.php'" class="btn_solid_pri col-6 mx-auto btn_lg" style="margin-top:50px">돌아가기</button>
