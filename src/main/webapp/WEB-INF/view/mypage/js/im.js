...
function loadXMLDoc(id, url, get_id) {
  $.ajaxSetup({
    cache: false, //close AJAX cache
  });
  $(id).load(url, { get_id: get_id });
}
//프린터
...
function im_notice(text, type = "success") {
  var msg;
  this.messages = {
    save: "저장 되었습니다.",
    delete: "삭제 되었습니다.",
  ...
  msg = this.messages[text];
  !msg && (msg = text);
  new NoticeJs({
    text: '<i class="fa fa fa-exclamation-circle m-r-xs"></i> ' + msg,
    type: type,
    position: "topCenter",
    animation: {
...
$(document)
  ...
  .on("change", '[data-toggle^="chageClass"]', function (e) {
    var $this = $(this),
      $class,
      $target,
      $tmp,
    ...
    if ($this.is(":input")) {
      is_checking_val = true;
      if ($this.is('input[type="checkbox"]')) {
        var id = $this.attr("id");
        var val = $(id).filter(":checked");
      }
    }
    ...
    $targets &&
      ...
      $.each($targets, function (index, value) {
        ...
        if (!val) {
          $targets[index] != "#" &&
            $($targets[index]).addClass($classes[index]);
        } else {
          $targets[index] != "#" &&
            $($targets[index]).removeClass($classes[index]);
        }
      });
  });
...
$(document).on("click", '[data-toggle="readonly"]', function (e) {
  ...
  if ($this.is('input[type="radio"]')) {
    $($target) && $($target).attr("readonly", false);
  } else {
    if ($checked) {
      $($target) && $($target).attr("readonly", false);
    } else {
  ...
  /* $target.$('#salary_month').removeAttr("readonly");
		$('#salary_year').attr("readonly", "readonly"); */
});
...
// 고정틀 기능 : 이름,환자명 화면 좌측 고정용
function stickyTableRow() {
  // console.log("stickyTableRow()")
  $(function () {
    // 로딩완료후 name-sticky 추가 해준다 (원래위치:arrange_index2.php. 문제없으면 해당위치가서 코드 삭제예정)

    if (
      null ==
      document
        .getElementsByClassName("table-responsive")[0]
        .getElementsByClassName("name-sticky")[0]
    ) {
      // console.log("TEST A");
      return false;
    }
    ...
    let index = document
      .getElementsByClassName("table-responsive")[0]
      .getElementsByClassName("name-sticky")[0];
    let rows = index.parentNode.children;
    ...
    for (var i = 0; i < tbody.children.length; i++) {
      tbody.children[i].children[index].classList.add("name-sticky");
    }
    // console.log("TEST F-1");
  });

  $(".table-list-container a.dropdown-toggle").on("click", function () {
    $.each($("td.name-sticky"), function (e, t) {
      t.style.zIndex = "1";
      // console.log("TEST D");
    });
    $(this).parent().parent().css("zIndex", 100);
    $(this)
      .closest("div.table-responsive")
      .css("cssText", `min-height: 550px !important`);
  });

  $("table.table .patient_dropdown_menu").click(function () {
    // console.log("TEST K");
    $("div.table-responsive .patient_dropdown_menu").css(
      "cssText",
  ...
  function removeStickyClass() {
    $("td.name-sticky").addClass("name-sticky-temp");
    $("td.name-sticky").removeClass("name-sticky");
    // console.log("TEST B");
  }

  function addStickyClass() {
    $("td.name-sticky-temp").addClass("name-sticky");
    $("td.name-sticky-temp").removeClass("name-sticky-temp");
    // console.log("TEST C");
  }
}
