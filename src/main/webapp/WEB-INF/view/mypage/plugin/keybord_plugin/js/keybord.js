...
// option.only_NFD_hangeul_encoding==1 : 완성형 낱내자(NFC)를 첫가끝 조합형 낱내자(NFD)로 바꿈
	...
	if(typeof f == 'undefined' || !f) f = document.getElementById('keybord_ko_input_text');
	var c,i,j,k;
...
function NFC_galmadeuli_preprocess(f,e,key) { // 유니코드 완성형 한글 부호계를 쓸 때의 갈마들이 세벌식 자판 전처리 함수 (신세벌식 자판을 기준으로 함)
	...
	if(ohi_c2<31 && with_shift_key(key) && !ohiQ[0] && !ohiQ[3] && ohiQ[6] && !ohiQ[7] && ohiDoubleJamo(2,ohiQ[6],ohi_c2)) {
...
function NFD_galmadeuli_preprocess(f,e,key) { // 첫가끝 조합형을 쓸 때의 갈마들이 세벌식 자판 전처리 함수 (신세벌식 자판을 기준으로 함)
	...
	if(option.enable_double_final_ext && with_shift_key(key) && sub_c1 
...
function hangeul_typewriter(f,key) { // 타자기 자판
	...
	if(Ko_type=='4t-1985') {
		if(shift_lock) {
			...
			if(unicode_ga.indexOf(c1)<0 || unicode_ga.indexOf(c2)<0) 
			{
				shift_lock=0;				
			}
...
function show_NCR_text(op) { // 문자를 유니코드 부호값과 맞대어 나타내기 (Numeric Character Reference)
	...
	var f = document.getElementById('keybord_ko_input_text');
...
function show_options() {
	...
	if(opts) {
		...
		if(!opt) {
			opt = appendChild(opts,'div','option','text_copy_button','<button onclick="copyToClipboard(document.getElementById(\'keybord_ko_input_text\'))">글 베끼기</button>');
...
function show_keyboard_layout(type) {
	...
	{		
		...
		current_layout_info.type_name = "2-KSX5002";
		ohiStart();
		en_keybord_init();
		$("#keyboardLayoutTable_EN").hide();
		$("#keyboardLayoutTable_KO").show();
	...
	else //en
	{
		...
		$("#keyboardLayoutTable_KO").hide();
		$("#keyboardLayoutTable_EN").show();
	}
 	...
 	for(i=0, k=-1; ue[i]; i++) {
		...
		for(j=0; ue[i][j]; j++) {
			...
			if(dh[i] && dh[i][j]) {
				...
				if(unicode_cheos.indexOf(charCode)>=0) tdclass = 'im_e1';
				else if(Ko_type.substr(1,2)=='t-' && charCode>=0x314F && charCode<0x3164) tdclass = 'h2 gin-hol';
				else if(unicode_ga.indexOf(charCode)>=0) tdclass = 'im_e1';
				else if(unicode_ggeut.indexOf(charCode)>=0) tdclass = 'im_e1';

				if(special_chars.indexOf(charCode)>=0) dh[i][j] = special_chars_string[special_chars.indexOf(charCode)];

				if(tdclass.substr(0,1)!='h')
					if(unicode_modern_ggeut.indexOf(uh[i][j].charCodeAt(0))>=0) tdclass = 'im_e1';
			}

			charCode = ue[i][j].charCodeAt(0);
			if(KE=='En' && ue[i][j].length==1)
				if(charCode>64 && charCode<91 || charCode>96 && charCode<123) tdclass = 'im_e1';
...
//			console.log(tdclass,tdid);
			...
			if(i==4) {
				...
				else col.style.width = '41px', col.className = 'im_e1 special';
			}
	...
	if(KE=='Ko' && (Ko_type=='3t-Oesol' || Ko_type=='4t-1985')) {
		...
		if(Ko_type=='4t-1985') {
			...
			document.getElementById('key25').className = 'im_h3';
			document.getElementById('key38').className = 'im_h3';
			document.getElementById('key39').className = 'im_h3';
		}
	}
...
function ohiStart() {
	var i;
	var textarea=document.getElementById('keybord_ko_input_text');
...
function ohiKeypress(e) {
	...
	if(f.id=='keybord_ko_input_text') {show_NCR_text();show_direct_typing_text();}
	if(option.turn_off_OHI) {
		tableKey_press(key);
		return false;
	}
...
function inputText_focus(f) {
	//console.log("inputText_focus");
	if(typeof f =='undefined') f=document.getElementById('keybord_ko_input_text');
	try{
		// trim left space
		$("#keybord_ko_input_text").val($("#keybord_ko_input_text").val().replace(/^\s+/, ""));
		if(f) f.focus();
	}
	catch(e)
	{
...
function en_keybord_init()
{	
	//console.log("en_keybord_init");

	$("#keyboardLayoutTable_EN button[id='key28']").css("background-color", "");
	$("#keyboardLayoutTable_EN button[id='key15']").html("q");
	$("#keyboardLayoutTable_EN button[id='key16']").html("w");
	$("#keyboardLayoutTable_EN button[id='key17']").html("e");
	$("#keyboardLayoutTable_EN button[id='key18']").html("r");
	$("#keyboardLayoutTable_EN button[id='key19']").html("t");
	$("#keyboardLayoutTable_EN button[id='key20']").html("y");
	$("#keyboardLayoutTable_EN button[id='key21']").html("u");
	$("#keyboardLayoutTable_EN button[id='key22']").html("i");
	$("#keyboardLayoutTable_EN button[id='key23']").html("o");
	$("#keyboardLayoutTable_EN button[id='key24']").html("p");
	
	$("#keyboardLayoutTable_EN button[id='key29']").html("a");
	$("#keyboardLayoutTable_EN button[id='key30']").html("s");
	$("#keyboardLayoutTable_EN button[id='key31']").html("d");
	$("#keyboardLayoutTable_EN button[id='key31']").html("f");
	$("#keyboardLayoutTable_EN button[id='key32']").html("g");
	$("#keyboardLayoutTable_EN button[id='key33']").html("h");
	$("#keyboardLayoutTable_EN button[id='key34']").html("i");
	$("#keyboardLayoutTable_EN button[id='key35']").html("j");
	$("#keyboardLayoutTable_EN button[id='key36']").html("k");
	$("#keyboardLayoutTable_EN button[id='key37']").html("l");

	$("#keyboardLayoutTable_EN button[id='key42']").html("z");
	$("#keyboardLayoutTable_EN button[id='key43']").html("x");
	$("#keyboardLayoutTable_EN button[id='key44']").html("c");
	$("#keyboardLayoutTable_EN button[id='key45']").html("v");
	$("#keyboardLayoutTable_EN button[id='key46']").html("b");
	$("#keyboardLayoutTable_EN button[id='key47']").html("n");
	$("#keyboardLayoutTable_EN button[id='key48']").html("m");
}

//var keybord_ko_arr={'key15':'ㅃ',name:'rose',gender:'女'}
function tableKey_clicked(e, key_num, dk, uk){
	inputText_focus();
	var key, f = document.getElementById('keybord_ko_input_text');
	var n=f.nodeName||f.tagName;
	//if(!f || n!='TEXTAREA') return false;
	console.log("type:"+current_layout_info.type_name);
	...
	var shift2 = document.getElementById('key52');
	//console.log("dk:"+dk);
	if(current_layout_info.type_name=="2-KSX5002")
	{
		shiftlock_click = 0;
		if(dk==16) {
			$("#keyboardLayoutTable_KO button[id='key15']").html("ㅃ");
			$("#keyboardLayoutTable_KO button[id='key16']").html("ㅉ");
			$("#keyboardLayoutTable_KO button[id='key17']").html("ㄸ");
			$("#keyboardLayoutTable_KO button[id='key18']").html("ㄲ");
			$("#keyboardLayoutTable_KO button[id='key19']").html("ㅆ");
			$("#keyboardLayoutTable_KO button[id='key23']").html("ㅒ");
			$("#keyboardLayoutTable_KO button[id='key24']").html("ㅖ");
		}
		else
		{			
			$("#keyboardLayoutTable_KO button[id='key15']").html("ㅂ");
			$("#keyboardLayoutTable_KO button[id='key16']").html("ㅈ");
			$("#keyboardLayoutTable_KO button[id='key17']").html("ㄷ");
			$("#keyboardLayoutTable_KO button[id='key18']").html("ㄱ");
			$("#keyboardLayoutTable_KO button[id='key19']").html("ㅅ");
			$("#keyboardLayoutTable_KO button[id='key23']").html("ㅐ");
			$("#keyboardLayoutTable_KO button[id='key24']").html("ㅔ");
		}	
	}
	else
	{
	...
	if(ohi_KE.substr(0,2)!='En' && key>32 && key<127) {
		...
		if(KE=='Ko') {
			//console.log(current_layout_info.type_name);
			if(current_layout_info.type_name.substr(0,2)=='2-') ohiHangeul2(f,e,key);
			else if(!ohiHangeul3_abbreviation(f,key)) ohiHangeul3(f,e,key);
		}
	}
