<?php
include_once('./_common_k.php');
include_once(IM_ORDER_PATH . '/lib.php');
include_once(IM_ORDER_PATH . '/lib.pay.php');
include_once(IM_PATH . '/kiosk/lib.kiosk.php');
include_once(IM_PATH . '/head.kiosk.php');
...
$orderNumber 	= $_REQUEST["wp_orderno"]; 						// 가맹점 주문번호(가맹점에서 직접 설정)
$price 			= $_REQUEST["wp_price"];        	    		// 상품가격(특수기호 제외, 가맹점에서 직접 설정)
...
$qry  .= "   tid, tr_stat, result_code, result_msg, moid, pay_method, tot_price, good_name, buyer_email, buyer_tel, buyer_name";
$qry  .= " ) values ( ";
$qry  .= " '{$timestamp}', 'R', '', '전문송신', '{$orderNumber}', 'CARD', '{$price}', '{$good_name}', '{$buyer_email}', '{$buyer_tel}', '{$buyer_name}' ";
$qry  .= " ) ";
...
<html lang="ko">
    ...
    <head>
		...
		<!--테스트 JS--><script language="javascript" type="text/javascript" src="https://tstdpay.paywelcome.co.kr/stdjs/INIStdPay.js" charset="UTF-8"></script>
		<!--운영 JS> <script language="javascript" type="text/javascript" src="https://stdpay.paywelcome.co.kr/stdjs/INIStdPay.js" charset="UTF-8"></script> -->
                ...
                <div class="card">
                    ...
                    <form name="" id="SendPayForm_id" method="post" class="mt-5 d-none">
                        <div class="row g-3 justify-content-between" style="--bs-gutter-x:0rem;">
                            ...
                            <input type="hidden" name="returnUrl" value="http://stage.denb.co.kr/plugin/wpay/wpay_pc_return.php">
                            <input type="hidden" name="closeUrl" value="http://stage.denb.co.kr/plugin/wpay/close.php">
